// Vercel serverless function — proxies the Instagram Graph API so the long-lived
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!igRes.ok) {
      res.status(200).json({ configured: true, posts: [], error: data?.error?.message || 'Instagram API error' })
      return
    }
    ...
    res.status(200).json({ configured: true, posts: [], error: err.message })
  }
}
